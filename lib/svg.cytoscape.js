...
//    =========================================================================
    ...
    Cytoscape.prototype.reset = function () {
        ...
        self.ui.svg = new SVG(self.ui.$.get(0));
        ...
        // Append the objects to SVG ui
        self.ui.SVG.container = SVGContainer;
        self.ui.SVG.lines = SVGLines;
        self.ui.SVG.draggableNodes = SVGDraggableNodes;
        self.ui.SVG.nodes = SVGNodes;
        self.ui.SVG.tmpLines = SVGTmpLines;
        self.ui.SVG.markers = SVGMarkers;

        // Init pan zoom interface
        self.panZoom = SVGContainer.panZoom({
            zoom: [0.05, 3]
        }).setPosition(graphData.pos.x, graphData.pos.y, graphData.zoom);

        // Listen for position changes
        SVGContainer.on("pan", updateOnPanOrZoom.bind(self));
        SVGContainer.on("zoom", updateOnPanOrZoom.bind(self));
        SVGContainer.on("zoom", self.zoomHandler.bind(self));
    };
