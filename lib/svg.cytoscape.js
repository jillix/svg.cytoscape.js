...
(function (root) {
    ...
    function SVGGraph(opt_options) {
        ...
        this.cytoscape = opt_options.cytoscape;
    }
     ...
     *
     * @param {Object} The raw element data.
     ...
     */
    SVGGraph.prototype.add = function (options, raw) {
        ...
        // Drag end
        drGroup.on("dragend", function (ev) {
            self.cytoscape.trigger("elementDragend", drGroup, ev);
        });
        ...
        conGroup.on("mouseenter", function () {
            ...
            self.cytoscape.trigger("elementMouseEnter", newElm);
        }).on("mouseleave", function () {
            conGroup.attr("data-mouseenter", "false");
            newElm.resetColor();
            self.cytoscape.trigger("elementMouseLeave", newElm);
        });
         ...
         *  - `raw` (Object) The raw element data.
         */
        newElm.addSubElm = function (data, raw) {

            var subElmGroup = data.cElm = self.options.cNodes.group()
              , className = "subelement";

            if (options.type) {
                className += " subelm-" + data.type + " parent-" + options.type;
            }

            subElmGroup.addClass(className)
            ...
            subElmGroup.on("mouseenter", function () {
                ...
                self.cytoscape.trigger("subelementMouseEnter", data, newElm, this);
            }).on("mouseleave", function () {
                subElmGroup.attr("data-mouseenter", "false");
                data.resetColor();
                self.cytoscape.trigger("subelementMouseLeave", data, newElm, this);
            });
    ...
    Cytoscape.prototype.reset = function () {
        ...
        self.dom.on("mousedown", "g.c-element, g.subelement", function () {
            svgLineJs.source = this.querySelector("circle");
            if (!svgLineJs.source) { return; }
        ...
        self.dom.on("mouseup", function (ev) {
            ...
            setTimeout(function() {
                ...
                if (svgLineJs.source === svgLineJs.target) {
                    self.trigger("elementClicked",  source, svgLineJs.source);
                } else if (source && target && svgLineJs.marker && svgLineJs.line) {
    ...
    Cytoscape.prototype.addSubElement = function (elm, subelm) {
        ...
        subelm.cyto.cElm.id(subelm.id).attr("data-parent-id", elm.id);
    };
