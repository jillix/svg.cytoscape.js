    ...
    Cytoscape.prototype.reset = function () {
        ...
        self.dom.on("mousemove", function (ev) {
            ...
            if (svgLineJs.line) {
                ...
                var marker = self.ui.SVG.markers.marker(10, 10);
                ...
                var translate = /translate\(\s*([^\s,)]+)[ ,]([^\s,)]+)/.exec(svgLineJs.source.parentNode.getAttribute("transform"));
                svgLineJs.x1 = parseFloat(translate[1]) + sPos.width / 2;
                svgLineJs.y1 = parseFloat(translate[2]) + sPos.height / 2;
                svgLineJs.mouse = {
                    x1: ev.clientX,
                    y1: ev.clientY
                };
                ...
                marker.id(markerId);
                svgLineJs.line = self.ui.SVG.tmpLines.line().attr("marker-end", "url(#" + markerId + ")");
    ...
    Cytoscape.prototype.render = function (data) {
        ...
        Object.keys(self.data.nodes).forEach(function (cNodeId) {
            ...
            if (cNode.subelms) {
                ...
                Object.keys(cNode.subelms).forEach(function (cSubElmId) {
                    if (!cNode.subelms[cSubElmId].lines) { return; }
